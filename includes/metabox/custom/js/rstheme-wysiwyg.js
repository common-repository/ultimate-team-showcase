 ...
 */
window.RS = window.RS || {};
window.RS.wysiwyg = window.RS.wysiwyg || {};
...
( function(window, document, $, wysiwyg, undefined ) {
	...
	function getGroupData( data ) {
		...
		if ( ! all[ groupid ] || ! all[ groupid ][ fieldid ] ) {
			...
			all[ groupid ][ fieldid ] = {
				template : wp.template( 'rs-wysiwyg-' + groupid + '-' + fieldid ),
				defaults : {

					// Get the data from the template-wysiwyg initiation.
					mce : $.extend( {}, tinyMCEPreInit.mceInit[ 'rs_i_' + groupid + fieldid ] ),
					qt  : $.extend( {}, tinyMCEPreInit.qtInit[ 'rs_i_' + groupid + fieldid ] )
				}
			};
			// This is the template-wysiwyg data, and we do not want that to be initiated.
			delete tinyMCEPreInit.mceInit[ 'rs_i_' + groupid + fieldid ];
			delete tinyMCEPreInit.qtInit[ 'rs_i_' + groupid + fieldid ];
		}
	...
	function initOptions( options ) {
		var nameRegex = new RegExp( 'rs_n_' + options.groupid + options.fieldid, 'g' );
		var idRegex   = new RegExp( 'rs_i_' + options.groupid + options.fieldid, 'g' );
	...
	/**
	 * Initializes all group wysiwyg editors. Hooked to rs_init.
	 *
	 * @since  2.2.3
	 *
	 * @return {void}
	...
	wysiwyg.initAll = function() {
		...
		$( '.rs-wysiwyg-placeholder' ).each( function() {
			$this = $( this );
			data  = $this.data();
		...
		if ( true === initiated ) {
			...
			$( document )
				.on( 'rs_add_row', wysiwyg.addRow )
				.on( 'rs_remove_group_row_start', wysiwyg.destroyRowEditors )
				.on( 'rs_shift_rows_start', wysiwyg.shiftStart )
				.on( 'rs_shift_rows_complete', wysiwyg.shiftComplete );
		}
	};

	/**
	 * Initiates wysiwyg editors in a new group row. Hooked to rs_add_row.
	 *
	 * @since  2.2.3
	 *
	...
	/**
	 * Destroys wysiwyg editors in a group row when that row is removed. Hooked to rs_remove_group_row_start.
	 *
	 * @since  2.2.3
	 *
	 * @param  {object} evt A jQuery-normalized event object.
	...
	wysiwyg.destroyRowEditors = function( evt, $btn ) {
		wysiwyg.destroy( $btn.parents( '.rs-repeatable-grouping' ).find( '.wp-editor-area' ).attr( 'id' ) );
	};
	...
	/**
	 * Initializes editors for a new RS row.
	 *
	 * @since  2.2.3
	 *
	 * @param  {object} $row A jQuery dom element object for the group row.
	...
	wysiwyg.initRow = function( $row, evt ) {
		...
		$row.find( '.rs-wysiwyg-inner-wrap' ).each( function() {
			$toReplace    = $( this );
			data          = $toReplace.data();
			defVal        = window.RS.getFieldArg( data.hash, 'default', '' );
			...
			data.value    = 'rs_add_row' !== evt.type && $toReplace.find( '.wp-editor-area' ).length ? $toReplace.find( '.wp-editor-area' ).val() : defVal;
	...
	wysiwyg.init = function( $toReplace, data, buttonsInit ) {
		...
		var mceActive = window.rs_l10.user_can_richedit && window.tinyMCE;
	...
	wysiwyg.destroy = function( id ) {
		if ( ! window.rs_l10.user_can_richedit || ! window.tinyMCE ) {
			// Nothing to see here.
			return;
		}
	...
	// Hook in our event callbacks.
	$( document ).on( 'rs_init', wysiwyg.initAll );

} )( window, document, jQuery, window.rs.wysiwyg );
