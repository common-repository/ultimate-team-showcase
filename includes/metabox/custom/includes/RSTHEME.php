<?php
/**
 * RSTHEME - The core metabox object
 *
 ...
 *
 * @property-read string $rstheme_id
...
/**
 * The main RSTHEME object for storing box data/properties.
 */
class RSTHEME extends RSTHEME_Base {
	...
	protected $mb_defaults = array(
		...
		'rstheme_styles'       => true, // Include RSTHEME stylesheet.
		'enqueue_js'       => true, // Include RSTHEME JS.
		'fields'           => array(),

		/*
		 * Handles hooking RSTHEME forms/metaboxes into the post/attachement/user/options-page screens
		...
		'show_in_rest'     => false,
		'classes'          => null, // Optionally add classes to the RSTHEME wrapper
		'classes_cb'       => '', // Optionally add classes to the RSTHEME wrapper (via a callback)
		...
		'display_cb'       => false, // Override the options-page form output (RSTHEME_Hookup::options_page_output()).
	...
	/**
	 * Whether there are fields to be shown in columns. Set in RSTHEME::add_field().
	 *
	 * @var   bool
	 * @since 2.2.2
	 */
	...
	public function __construct( $config, $object_id = 0 ) {
		...
		if ( empty( $config['id'] ) ) {
			wp_die( esc_html__( 'Metabox configuration is required to have an ID parameter.', 'ultimate-team-showcase' ) );
		}

		$this->rstheme_id = $config['id'];
		...
		RSTHEME_Boxes::add( $this );

		/**
		 * Hook during initiation of RSTHEME object
		 *
		 * The dynamic portion of the hook name, $this->rstheme_id, is this meta_box id.
		 *
		 * @param array $rstheme This RSTHEME object
		 */
		do_action( "rstheme_init_{$this->rstheme_id}", $this );

		// Hook in the hookup... how meta.
		add_action( "rstheme_init_hookup_{$this->rstheme_id}", array( 'RSTHEME_hookup', 'maybe_init_and_hookup' ) );

		// Hook in the rest api functionality.
		add_action( "rstheme_init_hookup_{$this->rstheme_id}", array( 'RSTHEME_REST', 'maybe_init_and_hookup' ) );
	}
	 ...
	 * 'rstheme_before_form' and "rstheme_before_{$object_type}_form_{$this->rstheme_id}"
	 *
	 * @since  2.2.0
	...
	public function render_form_open( $object_id = 0, $object_type = '' ) {
		 ...
		 * @param array  $rstheme         This RSTHEME object.
		 */
		do_action( 'rstheme_before_form', $this->rstheme_id, $object_id, $object_type, $this );
		 ...
		 * Could also be `comment`, `user` or `options-page`.
		 *
		 * The second dynamic portion of the hook name, $this->rstheme_id, is the meta_box id.
		 *
		 * @param array  $rsthemetheme_id      The current box ID
		 * @param int    $object_id   The ID of the current object
		 * @param array  $rsthemetheme         This RSTHEME object
		 */
		do_action( "rstheme_before_{$object_type}_form_{$this->rstheme_id}", $object_id, $this );

		echo '<div class="', esc_attr( $this->box_classes() ), '"><div id="rstheme-metabox-', sanitize_html_class( $this->rstheme_id ), '" class="rstheme-metabox rstheme-field-list">';

		return $this;
	}

	/**
	 * Defines the classes for the RSTHEME form/wrap.
	 *
	 * @since  2.0.0
	...
	public function box_classes() {
		...
		$classes = array( 'rstheme-wrap', 'form-table' );
		...
		if ( $this->is_alternate_context_box() ) {
			...
			if ( ! $this->prop( 'title' ) && ! $this->prop( 'remove_box_wrap' ) ) {
				$context[] = 'rstheme-context-wrap-no-title';
			}

			// Include a generic context wrapper.
			$context[] = 'rstheme-context-wrap';

			// Include a context-type based context wrapper.
			$context[] = 'rstheme-context-wrap-' . $this->prop( 'context' );

			// Include an ID based context wrapper as well.
			$context[] = 'rstheme-context-wrap-' . $this->prop( 'id' );
		 ...
		 * @since 2.2.2
		 *
		 * @param string $classes Array of classes for the rstheme-wrap.
		 * @param RSTHEME   $rstheme     This RSTHEME object.
		 */
		$classes = apply_filters( 'rstheme_wrap_classes', $classes, $this );
	 ...
	 * 'rstheme_after_form' and "rstheme_after_{$object_type}_form_{$this->rstheme_id}"
	 *
	 * @since  2.2.0
	 * @param  integer $object_id   Object ID.
	 * @param  string  $object_type Object type.
	...
	public function render_form_close( $object_id = 0, $object_type = '' ) {
		 ...
		 *
		 * The dynamic portion of the hook name, $this->rstheme_id, is the meta_box id.
		 *
		 * The first dynamic portion of the hook name, $object_type, is the type of object
		 * you are working with. Usually `post` (this applies to all post-types).
		 * Could also be `comment`, `user` or `options-page`.
		 ...
		 * @param array  $rstheme         This RSTHEME object
		 */
		do_action( "rstheme_after_{$object_type}_form_{$this->rstheme_id}", $object_id, $this );
		 ...
		 * @param array  $rstheme         This RSTHEME object.
		 */
		do_action( 'rstheme_after_form', $this->rstheme_id, $object_id, $object_type, $this );
	 ...
	 * @return mixed RSTHEME_Field object if successful.
	 */
	 ...
	 *
	 * @param array|RSTHEME_Field $args Array of field arguments for a group field parent or the group parent field.
	 * @return RSTHEME_Field|null Group field object.
	 */
	public function render_group( $args ) {
		$field_group = false;

		if ( $args instanceof RSTHEME_Field  ) {
	 ...
	 * @param  RSTHEME_Field $field_group The RSTHEME_Field group object.
	 *
	 * @return RSTHEME_Field|null Group field object.
	 */
	public function render_group_callback( $field_args, $field_group ) {
		...
		echo '<div class="rstheme-row rstheme-repeat-group-wrap ', esc_attr( $field_group->row_classes() ), '" data-fieldtype="group"><div class="rstheme-td"><div data-groupid="', esc_attr( $field_group->id() ), '" id="', esc_attr( $field_group->id() ), '_repeat" ', esc_attr( $this->group_wrap_attributes( $field_group ) ), '>';

		if ( $desc || $label ) {
		    $class = $desc ? ' rstheme-group-description' : '';
		    printf( "<div class='rstheme-row%s'><div class='rstheme-th'>", esc_attr( $class ) );
		    if ( $label ) {
		        printf( "<h2 class='rstheme-group-name'>%s</h2>", esc_html( $label ) );
		    }
		    if ( $desc ) {
		        printf( "<p class='rstheme-metabox-description'>%s</p>", esc_html( $desc ) );
		    }
		    echo '</div></div>';
		}
		...
		if ( $field_group->args( 'repeatable' ) ) {
		    echo '<div class="rstheme-row"><div class="rstheme-td"><p class="rstheme-add-row"><button type="button" data-selector="', esc_attr( $field_group->id() ), '_repeat" data-grouptitle="', esc_attr( $field_group->options( 'group_title' ) ), '" class="rstheme-add-group-row button-secondary">', esc_html( $field_group->options( 'add_button' ) ), '</button></p></div></div>';
		}
	 ...
	 *
	 * @since  2.2.3
	 * @param  RSTHEME_Field $field_group The group RSTHEME_Field object.
	 * @return string                  The attributes string.
	 */
	public function group_wrap_attributes( $field_group ) {
		$classes = 'rstheme-nested rstheme-field-list rstheme-repeatable-group';
		 ...
		 * @param RSTHEME_Field $field_group           The group rstheme_Field object.
		 */
		$group_wrap_attributes = apply_filters( 'rstheme_group_wrap_attributes', $group_wrap_attributes, $field_group );

		$atts = array();
		foreach ( $group_wrap_attributes as $att => $att_value ) {
			if ( ! RSTHEME_Utils::is_data_attribute( $att ) ) {
				$att_value = htmlspecialchars( $att_value );
			}

			$atts[ sanitize_html_class( $att ) ] = sanitize_text_field( wp_strip_all_tags( $att_value ) );
		}

		return RSTHEME_Utils::concat_attrs( $atts );
	}
	 ...
	 *
	 * @since  1.0.2
	 * @param  RSTHEME_Field $field_group     RSTHEME_Field group field object.
	 *
	 * @return RSTHEME
	 */
	public function render_group_row( $field_group ) {
		...
		echo '<div class="postbox rstheme-row rstheme-repeatable-grouping', esc_attr( $closed_class ), '" data-iterator="', esc_attr( $field_group->index ), '">';

		if ( $field_group->args( 'repeatable' ) ) {
			echo '<button type="button" data-selector="', esc_attr( $field_group->id() ), '_repeat" class="dashicons-before dashicons-no-alt rstheme-remove-group-row" title="', esc_attr( $field_group->options( 'remove_button' ) ), '"></button>';
		}

			echo '
			<div class="rsthemehandle" title="' , esc_attr__( 'Click to toggle', 'rstheme' ), '"><br></div>
			<h3 class="rstheme-group-title rsthemehandle-title"><span>', esc_html( $field_group->replace_hash( $field_group->options( 'group_title' ) ) ), '</span></h3>

			<div class="inside rstheme-td rstheme-nested rstheme-field-list">';
		...
		if ( $field_group->args( 'repeatable' ) ) {
			echo '
					<div class="rstheme-row rstheme-remove-field-row">
						<div class="rstheme-remove-row">
							<button type="button" data-selector="', esc_attr( $field_group->id() ), '_repeat" class="rstheme-remove-group-row rstheme-remove-group-row-button alignright button-secondary">', esc_html( $field_group->options( 'remove_button' ) ), '</button>
						</div>
					</div>
					';
		}
	 ...
	 *
	 * @since 2.0.0
	 * @param array           $field_args  Array of field arguments to be passed to RSTHEME_Field.
	 * @param RSTHEME_Field|null $field_group RSTHEME_Field group field object.
	 */
	public function add_hidden_field( $field_args, $field_group = null ) {
		if ( isset( $field_args['field_args'] ) ) {
			// For back-compatibility.
			$field = new RSTHEME_Field( $field_args );
		} else {
			$field = $this->get_new_field( $field_args, $field_group );
		}

		$types = new RSTHEME_Types( $field );
	...
	public function get_sanitized_values( array $data_to_sanitize ) {
		...
		// We do this So RSTHEME will sanitize our data for us, but not save it.
		$this->object_id( '_' );

		// Ensure temp. data store is empty.
		rstheme_options( 0 )->set();

		// We want to get any taxonomy values back.
		add_filter( "rstheme_return_taxonomy_values_{$this->rstheme_id}", '__return_true' );

		// Process/save fields.
		$this->process_fields();

		// Put things back the way they were.
		remove_filter( "rstheme_return_taxonomy_values_{$this->rstheme_id}", '__return_true' );

		// Get data from temp. data store.
		$sanitized_values = rstheme_options( 0 )->get_options();
	...
	/**
	 * Fires the "rstheme_{$object_type}_process_fields_{$rstheme_id}" action hook.
	 *
	 * @since  2.x.x
	 *
	 * @return RSTHEME
	...
	public function pre_process() {
		 ...
		 * @param int   $object_id The ID of the current object
		 */
		do_action( "rstheme_{$object_type}_process_fields_{$this->rstheme_id}", $this, $this->object_id() );

		return $this;
	}

	/**
	 * Fires the "rstheme_save_{$object_type}_fields" and
	 * "rstheme_save_{$object_type}_fields_{$rstheme_id}" action hooks.
	 *
	 * @since  2.x.x
	 *
	 * @return RSTHEME
	...
	public function after_save() {
		 ...
		 * @param array  $rstheme         This RSTHEME object
		 */
		do_action( "rstheme_save_{$object_type}_fields", $object_id, $this->rstheme_id, $this->updated, $this );
		 ...
		 * @param array  $rstheme         This RSTHEME object
		 */
		do_action( "rstheme_save_{$object_type}_fields_{$this->rstheme_id}", $object_id, $this->updated, $this );
	 ...
	 * @return mixed        Return of RSTHEME_Field::update_data().
	 */
	...
	public function save_group_field( $field_group ) {
		...
		foreach ( array_values( $field_group->fields() ) as $field_args ) {
			...
			foreach ( (array) $group_vals as $field_group->index => $post_vals ) {
				...
				$is_updated = ( ! RSTHEME_Utils::isempty( $new_val ) && $new_val !== $old_val );
				$is_removed = ( RSTHEME_Utils::isempty( $new_val ) && ! RSTHEME_Utils::isempty( $old_val ) );
			...
			$saved[ $field_group->index ] = RSTHEME_Utils::filter_empty( $saved[ $field_group->index ] );
		}// End foreach().

		$saved = RSTHEME_Utils::filter_empty( $saved );
	...
	public function object_id( $object_id = 0 ) {
		...
		switch ($this->object_type()) {
		    case 'user':
		        $object_id = isset($_REQUEST['user_id']) ? wp_unslash(sanitize_text_field($_REQUEST['user_id'])) : $object_id;
		        $object_id = empty($object_id) && 'user-new.php' !== $pagenow && isset($GLOBALS['user_ID']) ? $GLOBALS['user_ID'] : $object_id;
		        break;

		    case 'comment':
		        $object_id = isset($_REQUEST['c']) ? wp_unslash(sanitize_text_field($_REQUEST['c'])) : $object_id;
		        $object_id = empty($object_id) && isset($GLOBALS['comments']->comment_ID) ? $GLOBALS['comments']->comment_ID : $object_id;
		        break;

		    case 'term':
		        $object_id = isset($_REQUEST['tag_ID']) ? wp_unslash(sanitize_text_field($_REQUEST['tag_ID'])) : $object_id;
		        break;
	...
	public function box_types( $fallback = array() ) {
		return RSTHEME_Utils::ensure_array( $this->prop( 'object_types' ), $fallback );
	}
	 ...
	 *
	 * @since  2.0.3
	 * @param  string|array|RSTHEME_Field $field        Metabox field id or field config array or RSTHEME_Field object.
	 * @param  RSTHEME_Field|null         $field_group  (optional) RSTHEME_Field object (group parent).
	 * @param  bool                    $reset_cached (optional) Reset the internal cache for this field object.
	 *                                               Use sparingly.
	 *
	 * @return RSTHEME_Field|false                     RSTHEME_Field object (or false).
	 */
	public function get_field( $field, $field_group = null, $reset_cached = false ) {
		if ( $field instanceof RSTHEME_Field ) {
			return $field;
		}
		...
		$this->fields[ $index ] = new RSTHEME_Field( $this->get_field_args( $field_id, $field, $sub_field_id, $field_group ) );

		return $this->fields[ $index ];
	}

	/**
	 * Handles determining which type of arguments to pass to RSTHEME_Field
	 *
	 * @since  2.0.7
	 * @param  mixed           $field_id     Field (or group field) ID.
	 * @param  mixed           $field_args   Array of field arguments.
	 * @param  mixed           $sub_field_id Sub field ID (if field_group exists).
	 * @param  RSTHEME_Field|null $field_group  If a sub-field, will be the parent group RSTHEME_Field object.
	 * @return array                         Array of RSTHEME_Field arguments.
	 */
	...
	/**
	 * Get default field arguments specific to this RSTHEME object.
	 *
	 * @since  2.2.0
	 * @param  array      $field_args  Metabox field config array.
	 * @param  RSTHEME_Field $field_group (optional) RSTHEME_Field object (group parent).
	...
	public function add_field( array $field, $position = 0 ) {
		...
		switch ( $field['type'] ) {
			...
			case 'file_list':
				...
				add_filter( 'wp_prepare_attachment_for_js', array( 'RSTHEME_Type_File_Base', 'prepare_image_sizes_for_js' ), 10, 3 );
				break;
			...
			case 'group':
				...
				break;
			case 'colorpicker':
				// https://github.com/JayWood/RSTHEME_RGBa_Picker
				// Dequeue the rgba_colorpicker custom field script if it is used,
				// since we now enqueue our own more current version.
				add_action( 'admin_enqueue_scripts', array( 'RSTHEME_Type_Colorpicker', 'dequeue_rgba_colorpicker_script' ), 99 );
				break;
		}
	...
	protected function _add_field_to_array( $field, &$fields, $position = 0 ) {
		if ( $position ) {
			RSTHEME_Utils::array_insert( $fields, array( $field['id'] => $field ), $position );
		} else {
	...
	/**
	 * Handles metabox property callbacks, and passes this $rstheme object as property.
	 *
	 * @since  2.2.3
	...
	public function nonce() {
		if ( ! $this->generated_nonce ) {
			$this->generated_nonce = sanitize_html_class( 'nonce_' . basename( __FILE__ ) . $this->rstheme_id );
		}
