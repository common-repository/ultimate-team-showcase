 ...
 */
class RSTHEME_hookup extends RSTHEME_Hookup_Base {
	...
	/**
	 * RSTHEME taxonomies array for term meta
	 *
	 * @var   array
	 * @since 2.2.0
	 */
	...
	/**
	 * Array of RSTHEME_Options_Hookup instances if options page metabox.
	 *
	 * @var   RSTHEME_Options_Hookup[]|null
	 * @since 2.2.5
	 */
	...
	public function universal_hooks() {
		foreach ( get_class_methods( 'RSTHEME_Show_Filters' ) as $filter ) {
			add_filter( 'rstheme_show_on', array( 'RSTHEME_Show_Filters', $filter ), 10, 3 );
		}

		if ( is_admin() ) {
			// register our scripts and styles for rstheme
	...
	public function post_hooks() {
		...
		$context = $this->rstheme->prop( 'context' ) ? $this->rstheme->prop( 'context' ) : 'normal';
		...
		if ( $this->rstheme->has_columns ) {
			foreach ( $this->rstheme->box_types() as $post_type ) {
	...
	public function term_hooks() {
		if ( ! function_exists( 'get_term_meta' ) ) {
			wp_die( esc_html__( 'Term Metadata is a WordPress 4.4+ feature. Please upgrade your WordPress install.', 'ultimate-team-showcase' ) );
		}

		if ( ! $this->rstheme->prop( 'taxonomies' ) ) {
			wp_die( esc_html__( 'Term metaboxes configuration requires a "taxonomies" parameter.', 'ultimate-team-showcase' ) );
		}

		$this->taxonomies = (array) $this->rstheme->prop( 'taxonomies' );
		$show_on_term_add = $this->rstheme->prop( 'new_term_section' );
		...
		foreach ( $this->taxonomies as $taxonomy ) {
			 ...
			 *
			 * The dynamic portion of the hook name, $rstheme_id, is the metabox id.
			 *
			 * @param bool   $show_on_add Default is the value of the new_term_section rstheme parameter.
			 * @param object $rstheme         The RSTHEME instance
			 */
			$show_on_add = apply_filterstheme( "rstheme_show_on_term_add_form_{$this->rstheme->rstheme_id}", $show_on_add, $this->rstheme );
	...
	public function options_page_hooks() {
		$option_keys = $this->rstheme->options_page_keys();

		if ( ! empty( $option_keys ) ) {
			foreach ( $option_keys as $option_key ) {
				$this->options_hookup[ $option_key ] = new RSTHEME_Options_Hookup( $this->rstheme, $option_key );
	...
	protected static function register_styles() {
		...
		/**
		 * Filters the registered style dependencies for the rs stylesheet.
		 *
		 * @param array $dependencies The registered style dependencies for the rs stylesheet.
		 */
		$dependencies = apply_filters( 'rstheme_style_dependencies', array() );
		// Define the version number
		$rstheme_version = '1.0.5';
		wp_register_style( 'uteam-rstheme-styles', RSTHEME_Utils::url( "css/rstheme{$front}{$rtl}{$min}.css" ), $dependencies, $rstheme_version );
		wp_register_style( 'rstheme-display-styles', RSTHEME_Utils::url( "css/rstheme-display{$rtl}{$min}.css" ), $dependencies, $rstheme_version );

		self::$css_registration_done = true;
	}

	/**
	 * Registers scripts for RSTHEME
	 *
	 * @since  2.0.7
	 */
	protected static function register_js() {
		...
		add_action( $hook, array( 'RSTHEME_JS', 'enqueue' ), 8 );

		self::$js_registration_done = true;
	}

	/**
	 * Registers scripts and styles for RSTHEME
	 *
	 * @since  1.0.0
	 */
	...
	/**
	 * Enqueues scripts and styles for RSTHEME in admin_head.
	 *
	 * @since  1.0.0
	 */
	public function do_scripts( $hook ) {
		...
		if ( in_array( $hook, $hooks, true ) ) {
			if ( $this->rstheme->prop( 'rstheme_styles' ) ) {
				self::enqueue_rstheme_css();
			}
			if ( $this->rstheme->prop( 'enqueue_js' ) ) {
				self::enqueue_rstheme_js();
			}
		}
	}

	/**
	 * Register the RSTHEME field column headers.
	 *
	 * @since 2.2.2
	 */
	public function register_column_headers( $columns ) {
		$fields = $this->rstheme->prop( 'fields' );
	...
	/**
	 * The RSTHEME field column display output.
	 *
	 * @since 2.2.2
	 */
	public function column_display( $column_name, $object_id ) {
		if ( isset( $this->columns[ $column_name ] ) ) {
				$field = new RSTHEME_Field( array(
					'field_args'  => $this->columns[ $column_name ]['field'],
					'object_type' => $this->object_type,
					'object_id'   => $this->rstheme->object_id( $object_id ),
					'rstheme_id'      => $this->rstheme->rstheme_id,
				) );

				$this->rstheme->get_field( $field )->render_column();
		}
	}
	...
	/**
	 * Output the RSTHEME box/fields in an alternate context (not in a standard metabox area).
	 *
	 * @since 2.2.4
	 */
	public function add_context_metaboxes() {
		...
		foreach ( $this->rstheme->box_types() as $object_type ) {
	...
	public function output_context_metabox() {
		 ...
		 */
		$add_wrap = ! empty( $title ) || ! $this->rstheme->prop( 'remove_box_wrap' );
	...
	public function context_box_title_markup_open( $add_handle = true ) {
		...
		$page = get_current_screen()->id;
		add_filter( "postbox_classes_{$page}_{$this->rstheme->rstheme_id}", array( $this, 'postbox_classes' ) );

		
		echo '<div id="' . esc_attr( $this->rstheme->rstheme_id ) . '" class="' . esc_attr( postbox_classes( $this->rstheme->rstheme_id, $page ) ) . '">' . "\n";

		if ( $add_handle ) {
		    echo '<button type="button" class="handlediv button-link" aria-expanded="true">';
		    // translators: the function name
		    echo '<span class="screen-reader-text">' . sprintf( esc_html__( 'Toggle panel: %s' ), esc_html( $title ) ) . '</span>';
	...
	public function add_metaboxes() {
		...
		foreach ( $this->rstheme->box_types() as $object_type ) {
			if ( count( $this->rstheme->tax_metaboxes_to_remove ) ) {
				$this->remove_default_tax_metaboxes( $object_type );
			}

			add_meta_box( $this->rstheme->rstheme_id, $this->rstheme->prop( 'title' ), array( $this, 'metabox_callback' ), $object_type, $this->rstheme->prop( 'context' ), $this->rstheme->prop( 'priority' ) );
		}
	}
	...
	protected function remove_default_tax_metaboxes( $post_type ) {
		foreach ( $this->rstheme->tax_metaboxes_to_remove as $taxonomy ) {
	...
	public function postbox_classes( $classes ) {
		if ( $this->rstheme->prop( 'closed' ) && ! in_array( 'closed', $classes ) ) {
			$classes[] = 'closed';
		}

		if ( $this->rstheme->is_alternate_context_box() ) {
			$classes = $this->alternate_context_postbox_classes( $classes );
		} else {
			$classes[] = 'rstheme-postbox';
		}
	...
	protected function alternate_context_postbox_classes( $classes ) {
		$classes[] = 'context-box';
		$classes[] = 'context-' . $this->rstheme->prop( 'context' ) . '-box';

		if ( in_array( $this->rstheme->rstheme_id, get_hidden_meta_boxes( get_current_screen() ) ) ) {
			$classes[] = 'hide-if-js';
		}

		$add_wrap = $this->rstheme->prop( 'title' ) || ! $this->rstheme->prop( 'remove_box_wrap' );

		if ( $add_wrap ) {
			$classes[] = 'rstheme-postbox postbox';
		} else {
			$classes[] = 'rstheme-no-box-wrap';
		}
	...
	public function metabox_callback() {
		...
		$this->rstheme->show_form( $object_id, $this->object_type );
	}
	...
	public function user_new_metabox( $section ) {
		if ( $section == $this->rstheme->prop( 'new_user_section' ) ) {
			$object_id = $this->rstheme->object_id();
			$this->rstheme->object_id( isset( $_REQUEST['user_id'] ) ? $_REQUEST['user_id'] : $object_id );
			$this->user_metabox();
		}
	}
	...
	public function show_on() {
		 ...
		 * @param mixed  $rstheme           The RSTHEME instance
		 */
		$show = (bool) apply_filters( 'rstheme_show_on', $show, $this->rstheme->meta_box, $this->rstheme );

		return $show;
	}

	/**
	 * Get the RSTHEME priority property set to numeric hook priority.
	 *
	 * @since  2.2.0
	...
	public function get_priority( $default = 10 ) {
		$priority = $this->rstheme->prop( 'priority' );
	...
	public function save_post( $post_id, $post = false ) {
		...
		$this->rstheme->save_fields( $post_id, 'post', $_POST );
	}
	...
	public function save_comment( $comment_id ) {
		...
		if ( $this->can_save( get_comment_type( $comment_id ) ) && $can_edit ) {
			$this->rstheme->save_fields( $comment_id, 'comment', $_POST );
		}
	}
	...
	public function save_user( $user_id ) {
		...
		if ( $this->can_save( 'user' ) ) {
			$this->rstheme->save_fields( $user_id, 'user', $_POST );
		}
	}
	...
	public function save_term( $term_id, $tt_id, $taxonomy = '' ) {
		...
		if ( $this->taxonomy_can_save( $taxonomy ) && $this->can_save( 'term' ) ) {
			$this->rstheme->save_fields( $term_id, 'term', $_POST );
		}
	}
	...
	public function delete_term( $term_id, $tt_id, $taxonomy = '' ) {
		if ( $this->taxonomy_can_save( $taxonomy ) ) {
			...
			$this->rstheme->save_fields( $term_id, 'term', $data_to_delete );
		}
	}
	...
	public function can_save( $type = '' ) {
		...
		$can_save = (
			$this->rstheme->prop( 'save_fields' )
			// check nonce
			&& isset( $_POST[ $this->rstheme->nonce() ] )
			&& wp_verify_nonce( $_POST[ $this->rstheme->nonce() ], $this->rstheme->nonce() )
			// check if autosave
			&& ! ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE )
			// get the metabox types & compare it to this type
			&& ( $type && in_array( $type, $this->rstheme->box_types() ) )
		 ...
		 * @param object $rstheme      The RSTHEME instance
		 */
		return apply_filters( 'rstheme_can_save', $can_save, $this->rstheme );
	}

	/**
	 * Determine if taxonomy of term being modified is rstheme-editable.
	 *
	 * @since  2.2.0
	...
	/**
	 * Enqueues the 'rstheme-display-styles' if the conditions match (has columns, on the right page, etc).
	 *
	 * @since  2.2.2.1
	 */
	protected function maybe_enqueue_column_display_styles() {
		...
		if (
			$pagenow
			&& $this->rstheme->has_columns
			&& $this->rstheme->prop( 'rstheme_styles' )
			&& in_array( $pagenow, array( 'edit.php', 'users.php', 'edit-comments.php', 'edit-tags.php' ), 1 )
			) {
			self::enqueue_rstheme_css( 'rstheme-display-styles' );
		}
	}

	/**
	 * Includes RSTHEME styles
	 *
	 * @since  2.0.0
	 */
	public static function enqueue_rstheme_css( $handle = 'uteam-rstheme-styles' ) {

		/**
		 * Filter to determine if RSTHEME'S css should be enqueued.
		 *
		 * @param bool $enqueue_css Default is true.
		 */
		if ( ! apply_filters( 'rstheme_enqueue_css', true ) ) {
			return false;
		}
		 ...
		 */
		return wp_enqueue_style( 'rstheme-display-styles' === $handle ? $handle : 'uteam-rstheme-styles' );
	}
	 ...
	 * @since  2.0.0
	 */
	public static function enqueue_rstheme_js() {

		/**
		 * Filter to determine if RSTHEME'S JS should be enqueued.
		 *
		 * @param bool $enqueue_js Default is true.
		 */
		if ( ! apply_filters( 'rstheme_enqueue_js', true ) ) {
			return false;
		}
