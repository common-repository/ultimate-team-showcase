...
class RSTHEME_Ajax {
	...
	protected function __construct() {
		add_action( 'wp_ajax_rstheme_oembed_handler', array( $this, 'oembed_handler' ) );
		add_action( 'wp_ajax_nopriv_rstheme_oembed_handler', array( $this, 'oembed_handler' ) );
		// Need to occasionally clean stale oembed cache data from the option value.
		add_action( 'rstheme_save_options-page_fields', array( __CLASS__, 'clean_stale_options_page_oembeds' ) );
	}
	...
	public function oembed_handler() {
		...
		// Verify our nonce
		if ( ! ( isset( $_REQUEST['rstheme_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['rstheme_ajax_nonce'], 'ajax_nonce' ) ) ) {
			die();
		}
		...
		if ( empty( $oembed_string ) ) {
			wp_send_json_error( '<p class="ui-state-error-text">' . esc_html__( 'Please Try Again', 'ultimate-team-showcase' ) . '</p>' );
		}

		// Set width of embed
		$embed_width = !empty( sanitize_text_field( $_REQUEST['oembed_width'] ) ) && intval( $_REQUEST['oembed_width'] ) < 640 ? intval( $_REQUEST['oembed_width'] ) : '640';
		...
		$html = $this->get_oembed( array(
			...
			'object_type' => !empty( sanitize_text_field( $_REQUEST['object_type'] ) ) ? sanitize_text_field( $_REQUEST['object_type'] ) : 'post',
	...
	public function get_oembed( $args ) {
		...
		if ( $oembed['embed'] && $oembed['embed'] != $oembed['fallback'] ) {
			return '<div class="rstheme-oembed embed-status">' . $oembed['embed'] . '<p class="rstheme-remove-wrapper"><a href="#" class="rstheme-remove-file-button" rel="' . $oembed['args']['field_id'] . '">' . esc_html__( 'Remove Embed', 'ultimate-team-showcase' ) . '</a></p></div>';
		}
	...
	protected function cache_action( $meta_key ) {
		...
		if ( 'options-page' === $this->object_type ) {
			...
			$status = call_user_func_array( array( rs_options( $this->object_id ), $action ), $args );
		} else {
	...
	public static function clean_stale_options_page_oembeds( $option_key ) {
		$options = rstheme_options( $option_key )->get_options();
		$modified = false;
		if ( is_array( $options ) ) {
		...
		if ( $modified ) {
			$updated = rstheme_options( $option_key )->set( $options );
		}
	}
