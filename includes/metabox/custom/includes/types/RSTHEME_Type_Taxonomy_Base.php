<?php
/**
 * RSTHEME Taxonomy base field type
 *
 * @since  2.2.2
 *
 ...
 * @link      https://rs.io
 */
abstract class RSTHEME_Type_Taxonomy_Base extends RSTHEME_Type_Multi_Base {
	...
	public function non_post_object_terms() {
		...
		$cache_key = "rs-cache-{$taxonomy}-{$object_id}";
	...
	public function get_terms() {
		...
		return RSTHEME_Utils::wp_at_least( '4.5.0' )
		    ? get_terms( $args )
		    : get_terms( $this->field->args( 'taxonomy' ) ); // Remove the second parameter
	}
	...
	protected function no_terms_result( $error, $tag = 'li' ) {
		if ( is_wp_error( $error ) ) {
			...
			$message = $this->_text( 'no_terms_text', esc_html__( 'No terms', 'ultimate-team-showcase' ) );
			$data = '';
		}
	...
	protected function build_children( $parent_term, $saved ) {
		...
		if ( ! empty( $terms ) && is_array( $terms ) ) {
			$options = '<li class="rs-indented-hierarchy"><ul>';
