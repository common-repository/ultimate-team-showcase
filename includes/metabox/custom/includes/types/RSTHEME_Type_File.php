 ...
 * @link      https://rs.io
 */
class RSTHEME_Type_File extends RSTHEME_Type_File_Base {
	...
	public function render( $args = array() ) {
		...
		$a = $this->args = $this->parse_args( 'file', array(
			'class'           => 'rs-upload-file regular-text',
			'id'              => $this->_id(),
			'name'            => $this->_name(),
		...
		// This is not elegant, but compensates for RSTHEME_Types::_id
		...
		$output .= sprintf(
			'<input class="rs-upload-button button-secondary" type="button" value="%1$s" />',
			esc_attr( $this->_text( 'add_upload_file_text', esc_html__( 'Add or Upload File', 'ultimate-team-showcase' ) ) )
		);

		$output .= $a['desc'];
		$output .= $this->get_id_field_output();

		$output .= '<div id="' . $field->id() . '-status" class="rs-media-status">';
		if ( ! empty( $a['value'] ) ) {
	...
	public function get_file_preview_output() {
		...
		if ( $this->args['id_value'] ) {
			$image = wp_get_attachment_image( $this->args['id_value'], $this->args['preview_size'], null, array(
				'class' => 'rs-file-field-image',
			) );
		} else {
			$image = '<img style="max-width: ' . absint( $this->args['img_size_data']['width'] ) . 'px; width: 100%;" src="' . $this->args['value'] . '" class="rs-file-field-image" alt="" />';
		}
	...
	public function get_id_field() {
		...
		if ( $this->args['value'] && ! $id_value ) {
			$id_value = RSTHEME_Utils::image_id_from_url( esc_url_raw( $this->args['value'] ) );
		}
