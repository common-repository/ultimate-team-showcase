 ...
 * @link      https://rs.io
 */
class RSTHEME_Type_File_Base extends RSTHEME_Type_Text {
	...
	public function is_valid_img_ext( $file, $blah = false ) {
		$file_ext = RSTHEME_Utils::get_file_ext( $file );
		 ...
		 */
		$is_valid_types = apply_filters( 'rs_valid_img_types', $valid_types );
		 ...
		 */
		return (bool) apply_filters( "rs_{$field_id}_is_valid_img_ext", $is_valid, $file, $file_ext );
	}
	...
	public function img_status_output( $args ) {
		return sprintf( '<%1$s class="img-status rs-media-item">%2$s<p class="rs-remove-wrapper"><a href="#" class="rs-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>',
			$args['tag'],
			$args['image'],
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html( $this->_text( 'remove_image_text', esc_html__( 'Remove Image', 'ultimate-team-showcase' ) ) ),
	...
	public function file_status_output( $args ) {
		return sprintf( '<%1$s class="file-status rs-media-item"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="rs-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>',
			$args['tag'],
			esc_html( $this->_text( 'file_text', esc_html__( 'File:', 'ultimate-team-showcase' ) ) ),
			RSTHEME_Utils::get_file_name_from_path( $args['value'] ),
			$args['value'],
			esc_html( $this->_text( 'file_download_text', esc_html__( 'Download', 'ultimate-team-showcase' ) ) ),
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html( $this->_text( 'remove_text', esc_html__( 'Remove', 'ultimate-team-showcase' ) ) ),
			isset( $args['id_input'] ) ? $args['id_input'] : ''
		);
	}
	...
	public static function output_js_underscore_templates() {
		?>
		<script type="text/html" id="tmpl-rs-single-image">
			<div class="img-status rs-media-item">
				<img width="{{ data.sizeWidth }}" height="{{ data.sizeHeight }}" src="{{ data.sizeUrl }}" class="rs-file-field-image" alt="{{ data.filename }}" title="{{ data.filename }}" />
				<p><a href="#" class="rs-remove-file-button" rel="{{ data.mediaField }}">{{ data.stringRemoveImage }}</a></p>
			</div>
		</script>
		<script type="text/html" id="tmpl-rs-single-file">
			<div class="file-status rs-media-item">
				<span>{{ data.stringFile }} <strong>{{ data.filename }}</strong></span>&nbsp;&nbsp; (<a href="{{ data.url }}" target="_blank" rel="external">{{ data.stringDownload }}</a> / <a href="#" class="rs-remove-file-button" rel="{{ data.mediaField }}">{{ data.stringRemoveFile }}</a>)
			</div>
		</script>
		<script type="text/html" id="tmpl-rs-list-image">
			<li class="img-status rs-media-item">
				<img width="{{ data.sizeWidth }}" height="{{ data.sizeHeight }}" src="{{ data.sizeUrl }}" class="rs-file_list-field-image" alt="{{ data.filename }}">
				<p><a href="#" class="rs-remove-file-button" rel="{{ data.mediaField }}[{{ data.id }}]">{{ data.stringRemoveImage }}</a></p>
				<input type="hidden" id="filelist-{{ data.id }}" data-id="{{ data.id }}" name="{{ data.mediaFieldName }}[{{ data.id }}]" value="{{ data.url }}">
			</li>
		</script>
		<script type="text/html" id="tmpl-rs-list-file">
			<li class="file-status rs-media-item">
				<span>{{ data.stringFile }} <strong>{{ data.filename }}</strong></span>&nbsp;&nbsp; (<a href="{{ data.url }}" target="_blank" rel="external">{{ data.stringDownload }}</a> / <a href="#" class="rs-remove-file-button" rel="{{ data.mediaField }}[{{ data.id }}]">{{ data.stringRemoveFile }}</a>)
				<input type="hidden" id="filelist-{{ data.id }}" data-id="{{ data.id }}" name="{{ data.mediaFieldName }}[{{ data.id }}]" value="{{ data.url }}">
			</li>
		</script>
		<?php
	 ...
	 *
	 * Uses RSTHEME_Utils::get_named_size() to get the closest available named size
	 * from an array of width and height values and RSTHEME_Utils::get_available_image_sizes()
	...
	static function get_image_size_data( $img_size = '', $fallback = 'thumbnail' ) {
		...
		if ( is_array( $img_size ) ) {
			...
			if ( $named_size = RSTHEME_Utils::get_named_size( $img_size ) ) {
				$data['name'] = $named_size;
			}
		} else {

			$image_sizes = RSTHEME_Utils::get_available_image_sizes();
	...
	public static function prepare_image_sizes_for_js( $response, $attachment, $meta ) {
		foreach ( RSTHEME_Utils::get_available_image_sizes() as $size => $info ) {
