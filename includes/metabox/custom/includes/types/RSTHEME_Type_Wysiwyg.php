 ...
 * @method string _desc()
 */
class RSTHEME_Type_Wysiwyg extends RSTHEME_Type_Textarea {
	...
	public function render( $args = array() ) {
		...
		$field->add_js_dependencies( 'rs-wysiwyg' );
		...
		return $this->rendered(
			sprintf( '<div class="rs-wysiwyg-wrap">%s', parent::render( array(
				'class'         => 'rs_textarea rs-wysiwyg-placeholder',
	...
	public function add_wysiwyg_template_for_group() {
		...
		echo '<script type="text/template" id="tmpl-rs-wysiwyg-' . esc_attr( $group_id ) . '-' . esc_attr( $field_id ) . '">';
		// Need to wrap the template in a wrapper div w/ specific data attributes which will be used when adding/removing rows.
		echo '<div class="rs-wysiwyg-inner-wrap" data-iterator="{{ data.iterator }}" data-groupid="' . esc_attr( $group_id ) . '" data-id="' . esc_attr( $field_id ) . '" data-hash="' . esc_attr( $hash ) . '">' . esc_html( $editor ) . '</div>';
		echo '</script>';
	}
