 ...
 * @link      https://rs.io
 */
class RSTHEME_Type_File_List extends RSTHEME_Type_File_Base {
	...
	public function render( $args = array() ) {
		...
		$output .= parent::render( array(
			'type'  => 'hidden',
			'class' => 'rs-upload-file rs-upload-list',
			'size'  => 45,
			'desc'  => '',
			'value'  => '',
		...
		$output .= parent::render( array(
			'type'  => 'button',
			'class' => 'rs-upload-button button-secondary rs-upload-list',
			'value' => esc_attr( $this->_text( 'add_upload_files_text', esc_html__( 'Add or Upload Files', 'ultimate-team-showcase' ) ) ),
			'name'  => '',
			'id'  => '',
		) );

		$output .= '<ul id="' . $this->_id( '-status' ) . '" class="rs-media-status rs-attach-list">';
