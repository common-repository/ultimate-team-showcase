<?php
/**
 * RSTHEME Utility classes for handling multi-dimensional array data for options
 *
...
/**
 * Retrieves an instance of RSTHEME_Option based on the option key
 *
 * @package   RSTHEME
 * @author    RSTHEME team
 */
class RSTHEME_Options {
	/**
	 * Array of all RSTHEME_Option instances
	 *
	 * @var   array
	 * @since 1.0.0
	 */
	...
	public static function get( $option_key ) {
		...
		if ( empty( self::$option_sets ) || empty( self::$option_sets[ $option_key ] ) ) {
			self::$option_sets[ $option_key ] = new RSTHEME_Option( $option_key );
		}
...
class RSTHEME_Option {
	 ...
	 *
	 * @uses apply_filters() Calls 'rstheme_override_option_save_{$this->key}' hook
	...
	public function set( $options = array() ) {
		...
		$test_save = apply_filters( "rstheme_override_option_save_{$this->key}", 'rstheme_no_override_option_save', $this->options, $this );

		if ( 'rstheme_no_override_option_save' !== $test_save ) {
		 ...
		 * @param RSTHEME_Option $rstheme_option This object.
		 */
		$autoload = apply_filters( "rstheme_should_autoload_{$this->key}", true, $this );
	 ...
	 *
	 * @uses apply_filters() Calls 'rstheme_override_option_get_{$this->key}' hook to allow
	...
	public function get_options( $default = null ) {
		if ( empty( $this->options ) && ! empty( $this->key ) ) {
			...
			$test_get = apply_filters( "rstheme_override_option_get_{$this->key}", 'rstheme_no_override_option_get', $default, $this );

			if ( 'rstheme_no_override_option_get' !== $test_get ) {
