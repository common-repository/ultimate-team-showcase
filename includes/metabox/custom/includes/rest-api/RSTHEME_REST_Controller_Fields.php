<?php
/**
 * RSTHEME objects/fields endpoint for WordPres REST API.
 ...
 */
class RSTHEME_REST_Controller_Fields extends RSTHEME_REST_Controller_Boxes {
	...
	public function register_routes() {
		$args = array(
			'_embed' => array(
				'description' => __( 'Includes the box object which the fields are registered to in the response.', 'rstheme' ),
			),
			'_rendered' => array(
				'description' => __( 'When the \'_rendered\' argument is passed, the renderable field attributes will be returned fully rendered. By default, the names of the callback handers for the renderable attributes will be returned.', 'rstheme' ),
			),
			'object_id' => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'rstheme' ),
			),
			'object_type' => array(
				'description' => __( 'To view or modify the field\'s value, the \'object_id\' and \'object_type\' arguments are required.', 'rstheme' ),
			),
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<rstheme_id>[\w-]+)/fields/', array(
			array(
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<rstheme_id>[\w-]+)/fields/(?P<field_id>[\w-]+)', array(
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_access Whether this RSTHEME endpoint can be accessed.
		 * @param object $controller This RSTHEME_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'rstheme_api_get_fields_permissions_check', $can_access );
	}

	/**
	 * Get all public RSTHEME box fields.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function get_items( $request ) {
		...
		$fields = array();
		foreach ( $this->rest_box->rstheme->prop( 'fields', array() ) as $field ) {
	...
	public function get_item_permissions_check_filter( $can_access = true ) {
		 ...
		 * @param object $controller This RSTHEME_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'rstheme_api_get_field_permissions_check', $can_access );
	}

	/**
	 * Get one RSTHEME field from the collection.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function update_item_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_update Whether this RSTHEME endpoint can be accessed.
		 * @param object $controller This RSTHEME_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'rstheme_api_update_field_value_permissions_check', $can_update );
	}

	/**
	 * Update RSTHEME field value.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function update_item( $request ) {
		...
		if ( ! $this->request['value'] ) {
			return new WP_Error( 'rstheme_rest_update_field_error', __( 'RSTHEME Field value cannot be updated without the value parameter specified.', 'rstheme' ), array(
				'status' => 400,
			) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $can_delete Whether this RSTHEME endpoint can be accessed.
		 * @param object $controller This RSTHEME_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'rstheme_api_delete_field_value_permissions_check', $can_delete );
	}

	/**
	 * Delete RSTHEME field value.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function modify_field_value( $activity ) {
		...
		if ( ! $this->request['object_id'] || ! $this->request['object_type'] ) {
			return new WP_Error( 'rstheme_rest_modify_field_value_error', __( 'RSTHEME Field value cannot be modified without the object_id and object_type parameters specified.', 'rstheme' ), array(
				'status' => 400,
			) );
		}
		...
		if ( ! $this->field ) {
			return new WP_Error( 'rstheme_rest_no_field_by_id_error', __( 'No field found by that id.', 'rstheme' ), array(
				'status' => 403,
			) );
		}
		...
		if ( 'options-page' == $this->request['object_type'] ) {
			$this->field->args[ "value_{$activity}" ] = rstheme_options( $this->request['object_id'] )->set();
		}
	 ...
	 * @since 2.2.3
	 *
	 * @param  string\RSTHEME_Field Field id or Field object.
	 * @return WP_Error|WP_REST_Response
	 */
	 ...
	 * @since 2.2.3
	 *
	 * @param  RSTHEME_Field Field object.
	 ...
	 */
	protected function prepare_field_data( RSTHEME_Field $field ) {
		$field_data = array();
		...
		foreach ( $field_args as $key => $value ) {
			...
			if ( 'options_cb' === $key ) {
				$value = $field->options();
			} elseif ( in_array( $key, RSTHEME_Field::$callable_fields, true ) ) {
			...
			if ( empty( $value ) || is_scalar( $value ) || is_array( $value ) ) {
				...
				// translators: %s: Description of the object ID
				$field_data[ $key ] = sprintf( __( 'Value Error for %s', 'rstheme' ), $key );
			}
		}
	 ...
	 * @since  2.2.3
	 *
	 * @param  RSTHEME_Field $field Field object to build links from.
	 *
	 * @return array             Array of links
	 */
	protected function prepare_links( $field ) {
		$boxbase      = $this->namespace_base . '/' . $this->rest_box->rstheme->rstheme_id;
	...
	/**
	 * Checks if the RSTHEME box or field has any registered callback parameters for the given filter.
	 *
	 * The registered handlers will have a property name which matches the filter, except:
	 * - The 'rstheme_api' prefix will be removed
	...
	/**
	 * Unhooks any RSTHEME box or field registered callback parameters for the given filter.
	 *
	 * @since  2.2.3
	 *
	 * @param  string $filter The filter name.
