<?php
/**
 * RSTHEME objects/boxes endpoint for WordPres REST API.
 ...
 */
class RSTHEME_REST_Controller_Boxes extends RSTHEME_REST_Controller {
	...
	public function register_routes() {
		$args = array(
			'_embed' => array(
				'description' => __( 'Includes the registered fields for the box in the response.', 'rstheme' ),
			),
		);
		...
		$args['_rendered'] = array(
			'description' => __( 'Includes the fully rendered attributes, \'form_open\', \'form_close\', as well as the enqueued \'js_dependencies\' script handles, and \'css_dependencies\' stylesheet handles.', 'rstheme' ),
		);

		// Returns specific box's data.
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/(?P<rstheme_id>[\w-]+)', array(
			array(
				'methods'             => WP_REST_Server::READABLE,
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 */
		return apply_filters( 'rstheme_api_get_boxes_permissions_check', true, $this );
	}

	/**
	 * Get all public RSTHEME boxes.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	public function get_items( $request ) {
		...
		$boxes = RSTHEME_REST::get_all();
		if ( empty( $boxes ) ) {
			return new WP_Error( 'rstheme_rest_no_boxes', __( 'No boxes found.', 'rstheme' ), array(
				'status' => 403,
			) );
		}
	...
	public function get_item_permissions_check_filter( $can_access = true ) {
		 ...
		 * @param object $controller This RSTHEME_REST_Controller object.
		 */
		return $this->maybe_hook_callback_and_apply_filters( 'rstheme_api_get_box_permissions_check', $can_access );
	}

	/**
	 * Get one RSTHEME box from the collection.
	 *
	 * @since 2.2.3
	 *
	 * @param  WP_REST_Request $request Full data about the request.
	...
	/**
	 * Get a RSTHEME box prepared for REST
	 *
	 * @since 2.2.3
	 *
	 * @return array
	...
	public function get_rest_box() {
		$rstheme = $this->rest_box->rstheme;

		$boxes_data = $rstheme->meta_box;

		if ( isset( $this->request['_rendered'] ) && $this->namespace_base !== ltrim( RSTHEME_REST_Controller::get_intial_route(), '/' ) ) {
			$boxes_data['form_open'] = $this->get_cb_results( array( $rstheme, 'render_form_open' ) );
			$boxes_data['form_close'] = $this->get_cb_results( array( $rstheme, 'render_form_close' ) );
		...
		$response->add_links( $this->prepare_links( $rstheme ) );
	 ...
	 * @since  2.2.3
	 *
	 * @param  RSTHEME_REST $rstheme RSTHEME_REST object to build links from.
	 *
	 * @return array          Array of links
	 */
	protected function prepare_links( $rstheme ) {
		$boxbase      = $this->namespace_base . '/' . $rstheme->rstheme_id;
