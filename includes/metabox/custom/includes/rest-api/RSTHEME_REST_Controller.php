...
if ( ! class_exists( 'WP_REST_Controller' ) ) {
	...
	require_once rstheme_dir( 'includes/shim/WP_REST_Controller.php' );
}

/**
 * Creates RSTHEME objects/fields endpoint for WordPres REST API.
 ...
 */
abstract class RSTHEME_REST_Controller extends WP_REST_Controller {
	 ...
	 * @var string
	 */
	protected $namespace = RSTHEME_REST::NAME_SPACE;
	 ...
	 *
	 * Checks if a RSTHEME object callback property exists, and if it does,
	...
	public function maybe_hook_callback_and_apply_filters( $filter, $default_access ) {
		if ( ! $this->rest_box && $this->request->get_param( 'rstheme_id' ) ) {
			$this->rest_box = RSTHEME_REST::get_rest_box( $this->request->get_param( 'rstheme_id' ) );
		}
		 ...
		 * @since 2.2.3
		 *
		 * @param bool   $default_access Whether this RSTHEME endpoint can be accessed.
	...
	/**
	 * Checks if the RSTHEME box has any registered callback parameters for the given filter.
	 *
	...
	public function maybe_hook_registered_callback( $filter, $default_val ) {
		...
		$val = $this->rest_box->rstheme->maybe_hook_parameter( $filter, $default_val );
		if ( null !== $val ) {
			$default_val = $val;
		}
	...
	/**
	 * Unhooks any RSTHEME box registered callback parameters for the given filter.
	 *
	 * @since  2.2.3
	 *
	 * @param  string $filter The filter name.
	...
	public function maybe_unhook_registered_callback( $filter ) {
		...
		$this->rest_box->rstheme->maybe_hook_parameter( $filter, null, 'remove_filter' );
	}

	/**
	 * Prepare a RSTHEME object for serialization
	 *
	 * @since 2.2.3
	 *
	 * @param  mixed $data
	...
	/**
	 * Prepare the RSTHEME item for the REST response.
	 *
	 * @since 2.2.3
	 *
	...
	public function prepare_item_for_response( $data, $request = null ) {
		...
		/**
		 * Filter the prepared RSTHEME item response.
		 *
		 * @since 2.2.3
		 *
		 * @param mixed  $data           Prepared data
		 * @param object $request        The WP_REST_Request object
		 * @param object $rstheme_endpoints This endpoints object
		 */
		return apply_filters( 'rstheme_rest_prepare', rest_ensure_response( $data ), $this->request, $this );
	}
	...
	protected function initiate_rest_read_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_read ) {
			$this->rest_box = new WP_Error( 'rstheme_rest_no_read_error', __( 'This box does not have read permissions.', 'rstheme' ), array(
				'status' => 403,
			) );
		}
	}
	...
	protected function initiate_rest_edit_box( $request, $request_type ) {
		...
		if ( ! is_wp_error( $this->rest_box ) && ! $this->rest_box->rest_edit ) {
			$this->rest_box = new WP_Error( 'rstheme_rest_no_write_error', __( 'This box does not have write permissions.', 'rstheme' ), array(
				'status' => 403,
			) );
		}
	}
	...
	protected function initiate_rest_box( $request, $request_type ) {
		...
		if ( ! $this->rest_box ) {
			...
			$this->rest_box = new WP_Error( 'rstheme_rest_box_not_found_error', __( 'No box found by that id. A box needs to be registered with the "show_in_rest" parameter configured.', 'rstheme' ), array(
				'status' => 403,
			) );
			...
			if ( isset( $this->request['object_id'] ) ) {
				$this->rest_box->rstheme->object_id( sanitize_text_field( $this->request['object_id'] ) );
			}

			if ( isset( $this->request['object_type'] ) ) {
				$this->rest_box->rstheme->object_type( sanitize_text_field( $this->request['object_type'] ) );
			}
		}
	}
	...
	/**
	 * Get RSTHEME fields schema, conforming to JSON Schema
	 *
	 * @since 2.2.3
	 *
	 * @return array
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties'           => array(
				'description' => array(
					'description' => __( 'A human-readable description of the object.', 'rstheme' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),
				...
				'name' => array(
					'description' => __( 'The id for the object.', 'rstheme' ),
					'type'        => 'integer',
					'context'     => array(
						'view',
					),
				...
				'name' => array(
					'description' => __( 'The title for the object.', 'rstheme' ),
					'type'        => 'string',
					'context'     => array(
						'view',
					),
