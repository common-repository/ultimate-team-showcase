<?php
/**
 * RSTHEME Base - Base object functionality.
 *
 ...
 * @property-read $rstheme_id      Current RSTHEME instance ID
 * @property-read $object_id   Object ID
 * @property-read $object_type Type of object being handled. (e.g., post, user, comment, or term)
 */
abstract class RSTHEME_Base {

	/**
	 * Current RSTHEME instance ID
	 *
	 * @var   string
	 * @since 2.2.3
	 */
	protected $rstheme_id = '';
	...
	/**
	 * Get a new field object specific to this RSTHEME object.
	 *
	 * @since  2.2.0
	 * @param  array      $field_args  Metabox field config array.
	 * @param  RSTHEME_Field $field_group (optional) RSTHEME_Field object (group parent)
	 * @return RSTHEME_Field RSTHEME_Field object
	 */
	protected function get_new_field( $field_args, $field_group = null ) {
		return new RSTHEME_Field( $this->get_default_args( $field_args, $field_group ) );
	}

	/**
	 * Determine whether this rstheme object should show, based on the 'show_on_cb' callback.
	 *
	 * @since 2.0.9
	 *
	 * @return bool Whether this rstheme should be shown.
	 */
	public function should_show() {
		// Default to showing this rstheme
		$show = true;

		// Use the callback to determine showing the rstheme, if it exists
	...
	public function peform_param_callback( $param ) {
		print $this->get_param_callback_result( $param );
	}
	...
	public function maybe_hook_parameter( $hook_name, $val = null, $hook_function = 'add_filter' ) {
		...
		$parameter = substr( $hook_name, strlen( 'rstheme_api_' ) ) . '_cb';
	 ...
	 * @param string $version  The version of RSTHEME that deprecated the argument used.
	...
	protected function deprecated_param( $function, $version, $message = null ) {
		...
		if ( is_numeric( $message ) ) {
		    ...
		    switch ( $message ) {
		        ...
		        case self::DEPRECATED_PARAM:
		            // translators: %1$s is the deprecated parameter, %2$s is the new parameter.
		            $message = sprintf( __( 'The "%1$s" field parameter has been deprecated in favor of the "%2$s" parameter.', 'rstheme' ), $args[3], $args[4] );
		            break;

		        case self::DEPRECATED_CB_PARAM:
		            // translators: %1$s is the deprecated parameter, %2$s is the new parameter.
		            $message = sprintf( __( 'Using the "%1$s" field parameter as a callback has been deprecated in favor of the "%2$s" parameter.', 'rstheme' ), $args[3], $args[4] );
		            break;
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG && apply_filters( 'deprecated_argument_trigger_error', true ) ) {
			if ( function_exists( '__' ) ) {
			    if ( ! is_null( $message ) ) {
			        // translators: %1$s is the function name, %2$s is the version, %3$s is the deprecation message.
			        trigger_error( sprintf( esc_html__( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s! %3$s', 'ultimate-team-showcase' ), esc_html( $function ), esc_html( $version ), esc_html( $message ) ) );

			    } else {
			        // translators: %1$s is the function name, %2$s is the version.
			        trigger_error( sprintf( esc_html__( '%1$s was called with a parameter that is <strong>deprecated</strong> since version %2$s with no alternative available.', 'ultimate-team-showcase' ), esc_html( $function ), esc_html( $version ) ) );
	...
	public function __get( $field ) {
		switch ( $field ) {
			...
			default:
			    ...
			    esc_html__( 'Invalid %1$s property: %2$s', 'ultimate-team-showcase' ),
			    __CLASS__,
			    esc_html( $field )
			) );
		}
	...
	public function __call( $method, $args ) {
		...
		if ( ! has_filter( "{$object_class}_inherit_{$method}" ) ) {
			throw new Exception( sprintf(
			    // translators: %1$s is the class name, %2$s is the method or property name.
			    esc_html__( 'Invalid %1$s method: %2$s', 'ultimate-team-showcase' ),
