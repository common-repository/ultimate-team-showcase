...
/**
 * Show On Filters
 * Use the 'rstheme_show_on' filter to further refine the conditions
...
class RSTHEME_Show_Filters {
	 ...
	 * @param  array $meta_box_args Metabox config array
	 * @param  RSTHEME  $rstheme           The RSTHEME instance.
	 * @return bool                 Whether to display this metabox on the current page.
	 */
	public static function check_id( $display, $meta_box_args, $rstheme ) {
		...
		$object_id = is_admin() ? $rstheme->object_id() : get_the_ID();
	 ...
	 * @return bool            Whether to display this metabox on the current page.
	 */
	public static function check_page_template( $display, $meta_box_args, $rstheme ) {
		...
		$object_id = $rstheme->object_id();

		if ( ! $object_id || 'post' !== $rstheme->object_type() ) {
			return false;
		}
	...
	public static function check_admin_page( $display, $meta_box_args ) {
        ...
        $_wpnonce = wp_create_nonce('uteam_settings_page_nonce');
		...
		if ( is_admin() ) {
			...
			if ( ! isset( $_GET['page'] ) || ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( $_GET['_wpnonce'], 'uteam_settings_page_nonce' ) ) {
			    return $display;
			}
