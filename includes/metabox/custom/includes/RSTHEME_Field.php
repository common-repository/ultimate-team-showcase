 ...
 * @method mixed fields()
 */
class RSTHEME_Field extends RSTHEME_Base {
	...
	/**
	 * All RSTHEME_Field callable field arguments.
	...
	public function __construct( $args ) {
		...
		if ( ! empty( $args['group_field'] ) ) {
			...
			$this->rstheme_id      = $this->group->rstheme_id;
		} else {
			$this->object_id   = isset( $args['object_id'] ) && '_' !== $args['object_id'] ? $args['object_id'] : 0;
			$this->object_type = isset( $args['object_type'] ) ? $args['object_type'] : 'post';

			if ( isset( $args['rstheme_id'] ) ) {
				$this->rstheme_id = $args['rstheme_id'];
			}
		}
	...
	public function get_data( $field_id = '', $args = array() ) {
		 ...
		 * Returning a non 'rstheme_field_no_override_val' value
		 ...
		 * }
		 *
		 * @param RSTHEME_Field object $field This field object
		 */
		$data = apply_filters( 'rstheme_override_meta_value', 'rstheme_field_no_override_val', $this->object_id, $a, $this );

		/**
		 * Filter and parameters are documented for 'rstheme_override_meta_value' filter (above).
		 *
		 * The dynamic portion of the hook, $field_id, refers to the current
		 * field id paramater. Returning a non 'rstheme_field_no_override_val' value
		 * will effectively short-circuit the value retrieval.
		 *
		 * @since 2.0.0
		 */
		$data = apply_filters( "rstheme_override_{$a['field_id']}_meta_value", $data, $this->object_id, $a, $this );

		// If no override, get value normally
		if ( 'rstheme_field_no_override_val' === $data ) {
			$data = 'options-page' === $a['type']
				? rstheme_options( $a['id'] )->get( $a['field_id'] )
	...
	public function update_data( $new_value, $single = true ) {
		 ...
		 * @param RSTHEME_Field object $field This field object
		 */
		$override = apply_filters( 'rstheme_override_meta_save', null, $a, $this->args(), $this );

		/**
		 * Filter and parameters are documented for 'rstheme_override_meta_save' filter (above).
		 *
		 ...
		 * @since 2.0.0
		 */
		$override = apply_filters( "rstheme_override_{$a['field_id']}_meta_save", $override, $a, $this->args(), $this );
		...
		if ( 'options-page' === $a['type'] || empty( $a['id'] ) ) {
			return rstheme_options( $a['id'] )->update( $a['field_id'], $a['value'], false, $a['single'] );
		}
	...
	public function remove_data( $old = '' ) {
		 ...
		 * @param RSTHEME_Field object $field This field object
		 */
		$override = apply_filters( 'rstheme_override_meta_remove', null, $a, $this->args(), $this );
		 ...
		 * @param RSTHEME_Field object $field This field object
		 */
		$override = apply_filters( "rstheme_override_{$a['field_id']}_meta_remove", $override, $a, $this->args(), $this );
		...
		elseif ( 'options-page' === $a['type'] || empty( $a['id'] ) ) {
			return rstheme_options( $a['id'] )->remove( $a['field_id'] );
		}
	...
	public function sanitization_cb( $meta_value ) {
		...
		$sanitizer = new RSTHEME_Sanitize( $this, $meta_value );
		 ...
		 * @param object     $sanitizer  This `RSTHEME_Sanitize` object
		 */
		$override_value = apply_filters( "rstheme_sanitize_{$field_type}", null, $sanitizer->value, $this->object_id, $this->args(), $sanitizer );

		if ( null !== $override_value ) {
			return $override_value;
		}

		// Sanitization via 'RSTHEME_Sanitize'
	...
	public function save_field( $meta_value ) {
		...
		} elseif ( ! RSTHEME_Utils::isempty( $new_value ) && $new_value !== $this->get_data() ) {
			$updated = $this->update_data( $new_value );
			$action  = 'updated';
		} elseif ( RSTHEME_Utils::isempty( $new_value ) ) {
		 ...
		 * @param RSTHEME_Field object $field    This field object
		 */
		do_action( 'rstheme_save_field', $field_id, $updated, $action, $this );
		 ...
		 * @param RSTHEME_Field object $field   This field object
		 */
		do_action( "rstheme_save_field_{$field_id}", $updated, $action, $this );
	...
	public function repeatable_exception( $type ) {
		 ...
		 */
		$all_fields = array_merge( apply_filters( 'rstheme_non_repeatable_fields', array() ), $internal_fields );
	...
	public function has_args_method( $type ) {
		...
		$all_or_nothing_types = array_flip( apply_filters( 'rstheme_all_or_nothing_types', array(
			'select',
			'radio',
			'radio_inline',
			'taxonomy_select',
	...
	public function escaped_value( $func = 'esc_attr', $meta_value = '' ) {
		 ...
		 * @param array      $field_args The current field's arguments.
		 * @param object     $field      This `RSTHEME_Field` object.
		 */
		$esc = apply_filters( "rstheme_types_esc_{$field_type}", null, $meta_value, $this->args(), $this );
		if ( null !== $esc ) {
			return $esc;
		}
	...
	public function val_or_default( $meta_value ) {
		return ! RSTHEME_Utils::isempty( $meta_value ) ? $meta_value : $this->get_default();
	}
	...
	public function field_timezone_offset() {
		return RSTHEME_Utils::timezone_offset( $this->field_timezone() );
	}
	...
	public function get_timestamp_format( $format = 'date_format', $meta_value = 0 ) {
		...
		$meta_value = RSTHEME_Utils::make_valid_time_stamp( $meta_value );
	...
	public function get_timestamp_from_value( $value ) {
		return RSTHEME_Utils::get_timestamp_from_value( $value, $this->args( 'date_format' ) );
	}
	...
	public function render_field_callback() {
		...
		printf( "<div class=\"rstheme-row %s\" data-fieldtype=\"%s\">\n", esc_attr( $this->row_classes() ), esc_attr( $this->type() ) );
		...
		if ( ! $this->args( 'show_names' ) ) {
			...
			if ( $this->get_param_callback_result( 'label_cb' ) ) {
				...
				echo '<div class="rstheme-th">', esc_html( $this->peform_param_callback( 'label_cb' ) ), '</div>';
			}
	...
	/**
	 * Defines the classes for the current RSTHEME field row
	 *
	 * @since  2.0.0
	...
	public function row_classes() {
		 ...
		 */
		$repeat_table_rows_types = apply_filters( 'rstheme_repeat_table_row_types', array(
			'text_url',
			'text',
		) );

		$conditional_classes = array(
			'rstheme-type-' . str_replace( '_', '-', sanitize_html_class( $this->type() ) ) => true,
			'rstheme-id-' . str_replace( '_', '-', sanitize_html_class( $this->id() ) )    => true,
			'rstheme-repeat'             => $this->args( 'repeatable' ),
			'rstheme-repeat-group-field' => $this->group,
			'rstheme-inline'             => $this->args( 'inline' ),
		 ...
		 * @param RSTHEME_Field object $field   This field object
		 */
		return apply_filters( 'rstheme_row_classes', implode( ' ', $classes ), $this );
	}
	...
	public function display_value_callback() {
		...
		$display = new RSTHEME_Field_Display( $this );
		$field_type = $this->type();
		 ...
		 * @param RSTHEME_Field         $field      This field object.
		 * @param RSTHEME_Field_Display $display    The `RSTHEME_Field_Display` object.
		 */
		$pre_output = apply_filters( "rstheme_pre_field_display_{$field_type}", null, $this, $display );

		if ( null !== $pre_output ) {
			echo esc_html( $pre_output );
			return;
		}

		$this->peform_param_callback( 'before_display_wrap' );

		printf( "<div class=\"rstheme-column %s\" data-fieldtype=\"%s\">\n", esc_attr( $this->row_classes() ), esc_attr( $field_type ) );


		$this->peform_param_callback( 'before_display' );

		RSTHEME_Field_Display::get( $this )->display();
	...
	public function add_js_dependencies( $dependencies = array() ) {
		...
		RSTHEME_JS::add_dependencies( $dependencies );
	}
	...
	public function register_js_data() {
		if ( $this->group ) {
			RSTHEME_JS::add_field_data( $this->group );
		}

		return RSTHEME_JS::add_field_data( $this );
	}
	...
	public function hash_id() {
		if ( '' === $this->hash_id ) {
			$this->hash_id = RSTHEME_Utils::generate_hash( $this->rstheme_id . '||' . $this->id() );
		}
	...
	/**
	 * Get RSTHEME_Field default value, either from default param or default_cb param.
	 *
	 * @since  0.2.2
	 *
	...
	public function get_default() {
		...
		if ( null !== $default ) {
			return apply_filters( 'rstheme_default_filter', $default, $this );
		}
		...
		return apply_filters( 'rstheme_default_filter', $this->args['default'], $this );
	}
	...
	public function _set_field_defaults( $args ) {
		...
		$args['has_supporting_data'] = in_array(
			$args['type'],
			array(
				// RSTHEME_Sanitize::_save_file_id_value()/RSTHEME_Sanitize::_get_group_file_value_array()
				'file',
				// See RSTHEME_Sanitize::_save_utc_value()
				'text_datetime_timestamp_timezone',
			),
			true
		);
	...
	protected function set_field_defaults_group( $args ) {
		$args['options'] = wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'ultimate-team-showcase' ),
			'remove_button' => esc_html__( 'Remove Group', 'ultimate-team-showcase' ),
		) );
	...
	protected function set_field_defaults_all_or_nothing_types( $args ) {
		...
		$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'ultimate-team-showcase' ) : $args['show_option_none'];

		if ( null === $args['show_option_none'] ) {
			$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
			$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'ultimate-team-showcase' );
		}
	 ...
	 *                           arguments for RSTHEME_Field
	 *
	 * @return RSTHEME_Field         The new RSTHEME_Field instance.
	 */
	...
	/**
	 * Returns the RSTHEME instance this field is registered to.
	 *
	 * @since  2.2.2
	 *
	 * @return RSTHEME|WP_Error If new RSTHEME_Field is called without rstheme_id arg, returns error.
	 */
	public function get_rstheme() {
		if ( ! $this->rstheme_id ) {
			return new WP_Error( 'no_rstheme_id', esc_html__( 'Sorry, this field does not have a rstheme_id specified.', 'ultimate-team-showcase' ) );
		}

		return rstheme_get_metabox( $this->rstheme_id, $this->object_id, $this->object_type );
	}
