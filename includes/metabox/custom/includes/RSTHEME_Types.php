 ...
 *
 * @method string __id()
 * @method string __name()
 * @method string __desc()
 * @method string __text()
 * @method string __concat_attrs()
 */
class RSTHEME_Types {
	...
	/**
	 * Current RSTHEME_Field field object
	 *
	...
	/**
	 * Current RSTHEME_Type_Base object
	 *
	 * @var   RSTHEME_Type_Base object
	 * @since 2.2.2
	 */
	public $type = null;
	...
	/**
	 * Default fallback. Allows rendering fields via "rstheme_render_$fieldtype" hook
	 *
	 * @since 1.0.0
	...
	public function __call( $fieldtype, $arguments ) {
		...
		// Check for methods to be proxied to the RSTHEME_Type_Base object.
		 ...
		 *
		 * The dynamic portion of the hook name, $fieldtype, referstheme to the field type.
		 *
		 * @param array  $field              The passed in `RSTHEME_Field` object
		 ...
		 */
		do_action( "rstheme_render_{$fieldtype}", $this->field, $this->field->escaped_value(), $this->field->object_id, $this->field->object_type, $this );
	}
	...
	/**
	 * Proxies the method call to the RSTHEME_Type_Base object, if it exists, otherwise returns a default fallback value.
	 *
	 * @since  2.2.2
	 *
	 * @param  string $method  Method to call on the RSTHEME_Type_Base object.
	...
	/**
	 * If no RSTHEME_Types::$type object is initiated when a proxy method is called, it means
	 ...
	 * @since  2.2.3
	 *
	 * @param string $method  Method attempting to be called on the RSTHEME_Type_Base object.
	 */
	protected function guess_type_object( $method ) {
		...
		switch ( $method ) {
			...
			case 'concat_items':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_Select' );
				break;
			case 'is_valid_img_ext':
			case 'img_status_output':
			case 'file_status_output':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_File_Base' );
				break;
			case 'parse_picker_options':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_Text_Date' );
				break;
			case 'get_object_terms':
			case 'get_terms':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_Taxonomy_Multicheck' );
				break;
			case 'date_args':
			case 'time_args':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_Text_Datetime_Timestamp' );
				break;
			case 'parse_args':
				$this->get_new_render_type( $fieldtype, 'RSTHEME_Type_Text' );
				break;
		}
	...
	public function maybe_proxy_method( $method, $arguments ) {
		...
		if ( isset( $proxied[ $method ] ) ) {
			$exists = array(
				// Ok, proxy the method call to the RSTHEME_Type_Base object.
	...
	/**
	 * Checks for a custom field RSTHEME_Type_Base class to use for rendering.
	 *
	 * @since 2.2.4
	 *
	 * @param string $fieldtype Non-existent field type name.
	 ...
	 *
	 * @return bool|RSTHEME_Type_Base Type object if custom field is an object, false if field was added with
	 *   `rstheme_render_{$field_type}` action.
	 * @throws Exception if custom field type class does not extend RSTHEME_Type_Base.
	 */
	public function maybe_custom_field_object( $fieldtype, $args = array() ) {
		if ( $render_class_name = $this->get_render_type_class( $fieldtype ) ) {
			...
			if ( ! ( $this->type instanceof RSTHEME_Type_Base ) ) {
				
				throw new Exception( esc_html__( 'Custom RSTHEME field type classes must extend RSTHEME_Type_Base.', 'ultimate-team-showcase' ) );
	...
	/**
	 * Gets the render type RSTHEME_Type_Base object to use for rendering the field.
	 *
	 * @since  2.2.4
	 ...
	 * @return RSTHEME_Type_Base                    Type object.
	 */
	...
	public function get_render_type_class( $fieldtype, $render_class_name = null ) {
		...
		if ( has_action( "rstheme_render_class_{$fieldtype}" ) ) {

			/**
			 * Filters the custom field type class used for rendering the field. Class is required to extend RSTHEME_Type_Base.
			 *
			 ...
			 * @param object $field_type_object This `RSTHEME_Types` object.
			 */
			$render_class_name = apply_filters( "rstheme_render_class_{$fieldtype}", $render_class_name, $this );
		}
	...
	public function get_file_ext( $file ) {
		return RSTHEME_Utils::get_file_ext( $file );
	}
	...
	public function get_file_name_from_path( $value ) {
		return RSTHEME_Utils::get_file_name_from_path( $value );
	}
	...
	public function concat_attrs( $attrs, $attr_exclude = array() ) {
		return RSTHEME_Utils::concat_attrs( $attrs, $attr_exclude );
	}
	...
	public function render_repeatable_field() {
		...
		<div id="<?php echo esc_attr( $table_id ); ?>" class="rstheme-repeat-table rstheme-nested">
			<div class="rstheme-tbody rstheme-field-list">
				<?php $this->repeatable_rows(); ?>
			</div>
		</div>
		<p class="rstheme-add-row">
			<button type="button" data-selector="<?php echo esc_attr( $table_id ); ?>" class="rstheme-add-row-button button-secondary"><?php echo esc_html( $this->_text( 'add_row_text', esc_html__( 'Add Row', 'ultimate-team-showcase' ) ) ); ?></button>
		</p>
	 ...
	 */
	protected function repeat_row( $class = 'rstheme-repeat-row' ) {
		?>

		<div class="rstheme-row <?php echo esc_attr( $class ); ?>">
			<div class="rstheme-td">
				<?php $this->_render(); ?>
			</div>
			<div class="rstheme-td rstheme-remove-row">
				<button type="button" class="button-secondary rstheme-remove-row-button" title="<?php echo esc_attr( $this->_text( 'remove_row_button_title', esc_html__( 'Remove Row', 'ultimate-team-showcase' ) ) ); ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'ultimate-team-showcase' ) ) ); ?></button>
			</div>
		</div>
	...
	public function _desc( $paragraph = false, $echo = false, $repeat_group = false ) {
		...
		$desc = sprintf( "\n" . '<%1$s class="rstheme-metabox-description">%2$s</%1$s>' . "\n", $tag, $desc );
	...
	public function input( $args = array(), $type = __FUNCTION__ ) {
		return $this->get_new_render_type( 'text', 'RSTHEME_Type_Text', $args, $type )->render();
	}
	...
	public function textarea( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Textarea', $args )->render();
	}
	...
	public function hidden() {
		...
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text', $args, 'input' )->render();
	}

	public function text_small() {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text', array(
	...
	public function text_email() {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text', array(
			'class' => 'rstheme-text-email rstheme-text-medium',
			'type'  => 'email',
		), 'input' )->render();
	}
	...
	public function text_url() {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text', array(
			'class' => 'rstheme-text-url rstheme-text-medium regular-text',
			'value' => $this->field->escaped_value( 'esc_url' ),
		), 'input' )->render();
	}

	public function text_money() {
		$input = $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text', array(
			'class' => 'rstheme-text-money',
			'desc'  => $this->_desc(),
		), 'input' )->render();
	...
	public function textarea_small() {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Textarea', array(
			'class' => 'rstheme-textarea-small',
			'rows'  => 4,
		) )->render();
	}

	public function textarea_code( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Textarea_Code', $args )->render();
	}

	public function wysiwyg( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Wysiwyg', $args )->render();
	}

	public function text_date( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text_Date', $args )->render();
	}
	...
	public function text_time( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text_Time', $args )->render();
	}

	public function text_datetime_timestamp( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text_Datetime_Timestamp', $args )->render();
	}

	public function text_datetime_timestamp_timezone( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Text_Datetime_Timestamp_Timezone', $args )->render();
	}

	public function select_timezone( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Select_Timezone', $args )->render();
	}

	public function colorpicker( $args = array(), $meta_value = '' ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Colorpicker', $args, $meta_value )->render();
	}

	public function title( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Title', $args )->render();
	}

	public function select( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Select', $args )->render();
	}

	public function taxonomy_select( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Taxonomy_Select', $args )->render();
	}

	public function radio( $args = array(), $type = __FUNCTION__ ) {
		return $this->get_new_render_type( $type, 'RSTHEME_Type_Radio', $args, $type )->render();
	}
	...
	public function multicheck( $type = 'checkbox' ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Multicheck', array(), $type )->render();
	}
	...
	public function checkbox( $args = array(), $is_checked = null ) {
		...
		$render_class_name = $this->get_render_type_class( __FUNCTION__, 'RSTHEME_Type_Checkbox' );
		$this->type = new $render_class_name( $this, $args, $is_checked );
		return $this->type->render();
	}

	public function taxonomy_radio( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Taxonomy_Radio', $args )->render();
	}

	public function taxonomy_radio_hierarchical( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Taxonomy_Radio_Hierarchical', $args )->render();
	}
	...
	public function taxonomy_multicheck( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Taxonomy_Multicheck', $args )->render();
	}

	public function taxonomy_multicheck_hierarchical( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Taxonomy_Multicheck_Hierarchical', $args )->render();
	}
	...
	public function oembed( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_Oembed', $args )->render();
	}

	public function file_list( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_File_List', $args )->render();
	}

	public function file( $args = array() ) {
		return $this->get_new_render_type( __FUNCTION__, 'RSTHEME_Type_File', $args )->render();
	}
