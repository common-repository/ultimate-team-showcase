...
class RSTHEME_Sanitize {
	 ...
	 */
	public function __construct( RSTHEME_Field $field, $value ) {
		$this->field = $field;
		$this->value = $value;
	}
	...
	public function default_sanitization() {
		 ...
		 *
		 * @deprecated See documentation for "rstheme_sanitize_{$field_type}".
		 */
		if ( function_exists( 'apply_filters_deprecated' ) ) {
			$override_value = apply_filters_deprecated( "rstheme_validate_{$field_type}", array( null, $this->value, $this->field->object_id, $this->field->args(), $this ), '2.0.0', "rstheme_sanitize_{$field_type}" );
		} else {
			$override_value = apply_filters( "rstheme_validate_{$field_type}", null, $this->value, $this->field->object_id, $this->field->args(), $this );
		}
	 ...
	 *
	 * @since  2.2.4
	 * @return mixed  Blank value, or sanitized term values if "rstheme_return_taxonomy_values_{$rstheme_id}" is true.
	 */
	public function taxonomy() {
		$sanitized_value = '';

		if ( ! $this->field->args( 'taxonomy' ) ) {
			RSTHEME_Utils::log_if_debug( __METHOD__, __LINE__, "{$this->field->type()} {$this->field->_id()} is missing the 'taxonomy' parameter." );
		} else {
			...
			$rstheme_id = $this->field->rstheme_id;
			 ...
			 * (as they are stored as terms). This allows overriding that and is used by RSTHEME::get_sanitized_values().
			 *
			 * The dynamic portion of the hook, $rstheme_id, refers to the this field's RSTHEME box id.
			 *
			 * @since 2.2.4
			 *
			 * @param bool          $return_values By default, this is only true for 'options-page' boxes. To enable:
			 *                                     `add_filter( "rstheme_return_taxonomy_values_{$rstheme_id}", '__return_true' );`
			 * @param RSTHEME_Sanitize $sanitizer This object.
			 */
			if ( apply_filters( "rstheme_return_taxonomy_values_{$rstheme_id}", $return_values, $this ) ) {
				$sanitized_value = $this->_default_sanitization();
			}
		}
	...
	public function text_money() {
		...
		// See https://github.com/RSTHEME/RSTHEME/issues/1014
	...
	public function text_datetime_timestamp_timezone( $repeat = false ) {
		...
		if ( empty( $tzstring ) ) {
			$tzstring = RSTHEME_Utils::timezone_string();
		}

		$offset = RSTHEME_Utils::timezone_offset( $tzstring );
		...
		try {
			...
			$this->value = '';
			RSTHEME_Utils::log_if_debug( __METHOD__, __LINE__, $e->getMessage() );
		}
	...
	public function _save_file_id_value( $file_id_key ) {
		...
		if ( $this->value && ! $id_val ) {
			$id_val = RSTHEME_Utils::image_id_from_url( $this->value );
	...
	/**
	 * Returns a new, supporting, RSTHEME_Field object based on a new field id.
	 *
	 * @since  2.2.0
	 */
