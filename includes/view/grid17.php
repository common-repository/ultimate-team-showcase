...
$grid17 = '<div class="team-style post_'.esc_attr( $post_id ).'">
	<div class="team-grid-style">
		<div class="cl-row grid-style-17">';
			if ( $que->have_posts() ) {
				while ( $que->have_posts() ) : $que->the_post();
                    ...
                    $grid17 .= '<div class="cl-lg-'.esc_attr( $team_grid_colum ).' cl-sm-6 mb-20">
				        <div class="single-member-area">
			          			<div class="team-wrap">';
                    $grid17 .='<div class="image-wrap">';
                        $grid17 .='<div class="image-inner">';
			            			if ( has_post_thumbnail() ) {
                                        $grid17 .= '<a href="'.esc_url( $link ).'"><img src="' . esc_url( $member_image ) . '" alt="' .get_the_title(). '"></a>';
									}
                        $grid17 .='</div>';
                        ...
                        $grid17 .='<i class="share-i">'. $svg_icon.'</i>';

                        if( !empty( $member_social ) ){
                            $grid17 .='<div class="social-area">';
                            foreach ( $member_social as $team_social ) {
                                ...
                                if(!empty($s_link)):
                                    $grid17 .='<a href="'. esc_url($s_link).' " class="social-icon" target="_blank"><i class="'.esc_attr($s_icon).'"></i></a>';
                                endif;
                            }
                            $grid17 .='</div>';
                        }
                    $grid17 .='</div>';
                    $grid17 .='<div class="team-content">
                                <h3 class="team-name"><a href="'.esc_url( $link ).'" data-id="1" class="cl-single-item-popup">'.esc_html( $title ).'</a></h3>';

                                if( !empty($designation ) ){
                                    $grid17 .='<span class="team-title">'.esc_html( $designation ).'</span>';
                                }
...
$grid17 .= '<style>';
	if( !empty( $uteam_font_size_title ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .team-content .team-name{
			font-size: ".esc_html( $uteam_font_size_title ).";
		}";
	}
	if( !empty( $style_titlecolor ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .team-content .team-name a{
			color: ".esc_html( $style_titlecolor ).";
		}";
	}
	if( !empty( $uteam_overly_hover_titlecolor ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .team-content .team-name:hover a{
			color: ".esc_html( $uteam_overly_hover_titlecolor ).";
		}";
	}
	if( !empty( $uteam_font_size_designation ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .team-content .team-title{
	...
	if( !empty( $overlay_color ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .image-wrap:before{
			background-color: ".esc_html( $overlay_color ).";
		}";
	}
	if( !empty( $uteam_linkcolor ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .share-i svg{
			fill: ".esc_html( $uteam_linkcolor ).";
		}";
	}
	if( !empty( $uteam_link_bg_color ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .share-i{
			background-color: ".esc_html( $uteam_link_bg_color ).";
		}";
	}
	if( !empty( $uteam_icon_color ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .image-wrap .social-area .social-icon i{
	...
	if( !empty( $uteam_icon_hovercolor ) ){
        $grid17 .= ".post_".esc_attr( $post_id )." .team-grid-style .grid-style-17 .single-member-area .team-wrap .image-wrap .social-area .social-icon i:hover{
